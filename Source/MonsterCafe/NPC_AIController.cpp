...
void ANPC_AIController::on_target_detected(AActor* actor, FAIStimulus const stimulas)
{
	if (auto const ch = Cast<AMonsterCafeCharacter>(actor))
	{
		get_blackboard()->SetValueAsBool(bb_keys::can_see_player, stimulas.WasSuccessfullySensed());
	}
}

void ANPC_AIController::setup_perception_system()
{
	// create and initalise sight and configuration object
	...
	sight_config->PeripheralVisionAngleDegrees = 90.0f; // 360 would let the NPC to see all round him
